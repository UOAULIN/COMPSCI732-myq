        ...
        private void piece_Data()
        {
            ...
            foreach (string filePath in Directory.GetFiles(folderPathAllData))
            {
                ...
                foreach (string item in list)
                {
                    ...
                    string pieceAllDataPath = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "../../../PieceData"));

                    string piecefilePath = Path.Combine(pieceAllDataPath, piecefileName);
                    
                    byte[] byteArray = Encoding.UTF8.GetBytes(item);

                    Directory.CreateDirectory(piecefilePath);

                    File.WriteAllBytes(piecefilePath, byteArray);
